...
def backface_culling(p0, p1, p2, vv = [0,0, -1]): #view vector = <0,0,-1>, 3 polygons
    A = [p1[0] - p0[0], p1[1] - p0[1], p1[2] - p0[2]]
    B = [p2[0] - p0[0], p2[1] - p0[1], p2[2] - p0[2]]
    #surface normal
    AxB = [A[1]*B[2] - A[2]*B[1], A[2]*B[0] - A[0]*B[2], A[0]*B[1] - A[1]*B[0]]
    NdotV = AxB[0] * vv[0] + AxB[1] * vv[1] + AxB[2] * vv[2]
    return NdotV
...
def draw_polygons( points, screen, color ):
    c1 = 0
    while c1 < len(points):
        if backface_culling(points[c1], points[c1+1], points[c1+2]) < 0:
